...
*A brief for Claude Design. Everything needed to produce the full screen set
...
**The one-line promise:** *your class, your marks, your pages, your hours — in
one place.*
...
options, all ≥4.5:1 against their `on` value:

```
accent-1  Basil   #3F7D58 / soft #E6EFE9 / on #FFFFFF
accent-2  Indigo  #4A5FBF / soft #E7EAF7 / on #FFFFFF
accent-3  Ochre   #B57A1F / soft #F6EEDE / on #FFFFFF
accent-4  Rust    #B14A32 / soft #F7E9E5 / on #FFFFFF
accent-5  Teal    #2C7A82 / soft #E2EFF0 / on #FFFFFF
accent-6  Plum    #7A4A82 / soft #F0E8F2 / on #FFFFFF
```

**Highlighter colours.** These are the marks on the page and are a distinct
scale — they must read as translucent ink on white paper *and* stay visible
...
```
hl-yellow  #FFE27A / dark #8A7420   default label "grammar"
...
- **UI:** *Inter* (or Geist). Used for every control, label and navigation
  element.
- **Reading & note content:** *Source Serif 4* (or Literata). Used inside note
...
- **Mono:** *JetBrains Mono*, for code blocks in notes only.
...
- **Goal row:** one card per language, each with a progress **ring** (not a
  ...
  accent, and a pace line: *"1h 40m of 3h — on pace"* / *"behind by 45m"*.
...
accent-insensitivity note ("*perche* also finds *perché*").
...
- **Hitting a weekly goal** — the ring completes and holds a full accent fill
  for 1.5 s with a single soft pulse. One line: *"3h goal reached."* Nothing
  more. No confetti.
